...
class SingleRoIExtractor3D(nn.Module):
    ...
    def forward(self, feat, rois):
        if not isinstance(feat, tuple):
            feat = (feat, )
        # |_feat:tuple(tensor), tensor:(3,2048,8,16,16) & (3,256,32,16,16)
        if len(feat) >= 2:
        ...
        # |_feat:tuple(tensor), tensor:(3,2048,32,16,16) & (3,256,32,16,16)
        ...
        # |_feat:tuple(tensor), tensor:(3,2048,1,16,16) & (3,256,1,16,16)
        feat = torch.cat(feat, axis=1).contiguous()
        # |_feat: tensor:(3,2304,1,16,16)
        roi_feats = []
        for t in range(feat.size(2)):
            frame_feat = feat[:, :, t].contiguous()  # tensor:(3,2304,16,16)   rpis:相较于img而不是相较于feat的绝对坐标
            roi_feat = self.roi_layer(frame_feat, rois)  # (38,2304,8,8)
            if self.with_global:
                global_feat = self.global_pool(frame_feat.contiguous())
                inds = rois[:, 0].type(torch.int64)
                global_feat = global_feat[inds]
                roi_feat = torch.cat([roi_feat, global_feat], dim=1)  #original
                #roi_feat = roi_feat + global_feat
                roi_feat = roi_feat.contiguous()  
            roi_feats.append(roi_feat)  
        # roi_feats: [(38,2304,8,8)]  ->   (38,2304,1,8,8)
        return torch.stack(roi_feats, dim=2), feat  # 增加一个新的维度来拼接： [(3,3), [3,3], dim=0) -> (2,3,3)

...
if mmdet_imported:
    ...
    class monkey_AVARoIHead_switch(monkey_StandardRoIHead_switch):
        def switch_net(self,bbox_feat, rois, actor_cls):
            ...
            pass
            return status
        def feat2qkv2feat(self,status, bbox_feat):
            #status:(124,2)
            #bbox_featx:(124,2304,1,8,8)
            #->group_bbox_feat1: (124,512,8,8)  *2
            #->[group_bbox_feat1, group_bbox_feat2]
            gropu_bbox_feats = self.switch_group_qkv.forward(status, bbox_feat) 
            return gropu_bbox_feats  
        def qkv2score(self,switch_q, switch_k, switch_v):
            cls_score = None
            pass
            return cls_score
        ...
        def _bbox_forward(self, x, rois, img_metas):
            ...
            # bbox_feat: (110, 2304,1,8,8)   global_feat:(20,2304,1,16,16)  rois:(110,5)   x[0]:(20,2048,4,16,16)  x[1]:(20,256,16,16,16)
            actor_cls=0    #*****
            status = self.switch_net(bbox_feat, rois, actor_cls)  #(110,2)
            gropu_bbox_feats = self.feat2qkv2feat(status, bbox_feat)    #-> (124,512,1,8,8)
            ...
            cls_score, bbox_pred = self.bbox_head(gropu_bbox_feats)        
            bbox_results = dict(
                cls_score=cls_score, bbox_pred=bbox_pred, bbox_feats=bbox_feat)

            #cls_score = self.qkv2score(switch_q, switch_k, switch_v)    #***************
            # bbox_results = dict(
            #     cls_score=cls_score, bbox_pred=None, bbox_feats=bbox_feat)
            return bbox_results
    ...
    class monkey_AVARoIHead_switch:
        pass

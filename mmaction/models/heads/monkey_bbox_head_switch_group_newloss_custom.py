...
class monkey_BBoxHeadAVA_switch_group_newloss_custom(nn.Module):
    ...
    def __init__(
        ...
        super(monkey_BBoxHeadAVA_switch_group_newloss_custom, self).__init__()
        ...
        slow_classes = [1,  3,  5,6,7,8, 10,11,12,13,14,15   , 17, 18, 19]
        fast_classes = [  2,  4,      8,9,           14,15,16, 17, 18, 19]
        ...
        self.mix8 = nn.Linear(2, 1)
        self.mix14 = nn.Linear(2, 1)
        self.mix15 = nn.Linear(2, 1)
        self.mix17 = nn.Linear(2, 1)
        self.mix18 = nn.Linear(2, 1)
        self.mix19 = nn.Linear(2, 1)

        #index 8 14 15 17 18 19
        slow_classes_temp = list(self.slow_classes)
        fast_classes_temp = list(self.fast_classes)
        self.get_ind(slow_classes_temp, fast_classes_temp)

    def get_ind(self, slow_classes_temp, fast_classes_temp):
        self.ind8 = slow_classes_temp.index(8)
        self.ind14 = slow_classes_temp.index(14)
        self.ind15 = slow_classes_temp.index(15)
        if 17 in slow_classes_temp:
            self.ind17 = slow_classes_temp.index(17)
        else:
            self.ind17 = -1
        if 18 in slow_classes_temp:
            self.ind18 = slow_classes_temp.index(18)
        else:
            self.ind18 = -1
        if 19 in slow_classes_temp:
            self.ind19 = slow_classes_temp.index(19)
        else:
            self.ind19 = -1
        self.ind_f8 = fast_classes_temp.index(8)
        self.ind_f14 = fast_classes_temp.index(14)
        self.ind_f15 = fast_classes_temp.index(15)
        if 17 in fast_classes_temp:
            self.ind_f17 = fast_classes_temp.index(17)
        else:
            self.ind_f17 = -1
        if 18 in fast_classes_temp:
            self.ind_f18 = fast_classes_temp.index(18)
        else:
            self.ind_f18 = -1
        if 19 in fast_classes_temp:
            self.ind_f19 = fast_classes_temp.index(19)
        else:
            self.ind_f19 = -1
    ...
    def forward(self, x, status):
        ...
        for i in range(1, self.num_classes_fast):
            ...
            if actual_label not in [8, 14, 15, 18]:
                cls_score[:, actual_label] = cls_score_fast[:, i]
            else:
                # r = torch.rand(B,1) > 0.5
                # sr = torch.tensor(list(map(float, r))).cuda()
                if actual_label == 8:
                    slow = cls_score_slow[:, self.ind8:self.ind8+1]
                    fast = cls_score_fast[:, self.ind_f8:self.ind_f8+1]
                    out = torch.cat([slow, fast], dim=1)
                    out = self.mix8(out)
                elif actual_label == 14:
                    slow = cls_score_slow[:, self.ind14:self.ind14+1]
                    fast = cls_score_fast[:, self.ind_f14:self.ind_f14+1]
                    out = torch.cat([slow, fast], dim=1)
                    out = self.mix14(out)
                elif actual_label == 15:  # actual_label == 15
                    slow = cls_score_slow[:, self.ind15:self.ind15+1]
                    fast = cls_score_fast[:, self.ind_f15:self.ind_f15+1]
                    out = torch.cat([slow, fast], dim=1)
                    out = self.mix15(out)
                elif actual_label == 17:
                    slow = cls_score_slow[:, self.ind17:self.ind17+1]
                    fast = cls_score_fast[:, self.ind_f17:self.ind_f17+1]
                    out = torch.cat([slow, fast], dim=1)
                    out = self.mix17(out)
                elif actual_label == 18:
                    slow = cls_score_slow[:, self.ind18:self.ind18+1]
                    fast = cls_score_fast[:, self.ind_f18:self.ind_f18+1]
                    out = torch.cat([slow, fast], dim=1)
                    out = self.mix18(out)
                else:
                    slow = cls_score_slow[:, self.ind19:self.ind19+1]
                    fast = cls_score_fast[:, self.ind_f19:self.ind_f19+1]
                    out = torch.cat([slow, fast], dim=1)
                    out = self.mix19(out)
                #out = torch.max(slow, fast)
                # slow = torch.cat([cls_score_slow[:, 6:7], cls_score_slow[:, 11:12],cls_score_slow[:, 12:13]], dim=1)
                # fast = torch.cat([cls_score_fast[:, 3:4], cls_score_fast[:, 5:6],cls_score_fast[:, 6:7]], dim=1)
                #out = self.mix(out)
                cls_score[:, actual_label] = out.squeeze(1)
                # mul_result = torch.max(cls_score[:, actual_label], cls_score_fast[:, i])
                # cls_score[:, actual_label] = mul_result
...
if mmdet_imported:
    MMDET_HEADS.register_module()(monkey_BBoxHeadAVA_switch_group_newloss_custom)

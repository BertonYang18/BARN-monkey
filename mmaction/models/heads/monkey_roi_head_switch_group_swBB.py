...
try:
    ...
    from mmdet.models.roi_heads import monkey_StandardRoIHead_switch
    from mmdet.models.roi_heads import monkey_StandardRoIHead_switch_swBB
    mmdet_imported = True
...
if mmdet_imported:
    ...
    class monkey_AVARoIHead_switch_group_swBB(monkey_StandardRoIHead_switch_swBB):
        def switch_net(self, bbox_feat, rois, actor_cls):
            #switch_backbone = monkey_switch_ResNet3dSlowFast(pretrained=None)
            #switch = switch_backbone.forward(bbox_feat)
            status = self.switch_head.forward(bbox_feat)
            pass
            return status
        def feat2qkv2feat(self,status, rois, bbox_feat):
            #status:(124,2)
            #bbox_featx:(124,2304,1,8,8)
            #->group_bbox_feat1: (124,512,8,8)  *2
            #->[group_bbox_feat1, group_bbox_feat2]
            group_bbox_feats = self.switch_group_qkv.forward(status, rois, bbox_feat) 
            return group_bbox_feats  
        ...
        def _bbox_forward(self, x, rois, img_metas):
            ...
            """
            # rois:(110,5)   x&x_switch: ---x[0]:(20,2048,4,16,16)  x[1]:(20,256,16,16,16)
            x_main, x_switch = x
            status = 0
            # bbox_feat: (110, 2304,1,8,8)   global_feat:(20,2304,1,16,16)  
            bbox_feat, global_feat = self.bbox_roi_extractor(x_main, rois)
            # x_switch -> bbox_feat_switch -> status
            bbox_feat_switch, global_feat_switch = self.bbox_roi_extractor(x_switch, rois)
            actor_cls=0    #*****
            status = self.switch_net(bbox_feat_switch, rois, actor_cls)  #(110,2)

            # x_switch -> bbox_feat_switch -> status --|
            # x        -> bbox_feat                    + ->  group_qkv    ->  gropu_bbox_feats  
            #                                 rois   --|                         
            group_bbox_feats = self.feat2qkv2feat(status, rois, bbox_feat)    #-> (124,512,1,8,8)
            #group_bbox_feats = (bbox_feat, bbox_feat)  # baseline
            ...
            cls_score, bbox_pred = self.bbox_head(group_bbox_feats, status)   
            #cls_score, bbox_pred = self.bbox_head(group_bbox_feats, status)       
            bbox_results = dict(
    ...
    class monkey_AVARoIHead_switch_group_swBB:
        pass

...
class monkey_BBoxHeadAVA_switch_group_linear(nn.Module):
    ...
    def __init__(
        ...
        super(monkey_BBoxHeadAVA_switch_group_linear, self).__init__()
        ...
        slow_classes = [1,  3,  5,6,7,8, 10,11,12,13,14,15]
        fast_classes = [  2,  4,      8,9,           14,15,16]
        ...
        #self.fc_cls = nn.Linear(in_channels, custom_classes)
        ...
        #加权求和
        kernal_size_all = 3
        padding = kernal_size_all // 3
        self.conv_all1 = nn.Conv2d(in_channels, 512, kernal_size_all, padding, bias=None)
        self.conv_all2 = nn.Conv2d(in_channels, 512, kernal_size_all, padding, bias=None)
        self.fc_cls_all1 = nn.Linear(512, num_classes)
        self.fc_cls_all2 = nn.Linear(512, num_classes)
    ...
    #     x_fast = self.temporal_pool(x_fast)
    #     x_fast = self.spatial_pool(x_fast)
    #     if not self.dropout_before_pool and self.dropout_ratio > 0:
    #         x_fast = self.dropout(x_fast)
    ...
    #     cls_score = torch.tensor([100.]*self.num_classes_all).reshape(1,self.num_classes_all).repeat(B, 1).cuda() #->(B, num_classes)
    #     cls_score_slow = self.fc_cls1(x)
    #     cls_score_fast = self.fc_cls2(x_fast)
    ...
    def forward(self, x, status):
        ...
        x = x.view(x.size(0), -1)
        x_score_slow = self.fc_cls_all1(x)

        #fast
        x_fast = self.conv_all2(x_fast)
        if self.dropout_before_pool and self.dropout_ratio > 0:
            x_fast = self.dropout(x_fast)
        #x_fast = self.temporal_pool(x_fast)
        x_fast = self.spatial_pool(x_fast)
        ...
        x_fast = x_fast.view(x_fast.size(0), -1)
        x_score_fast = self.fc_cls_all2(x_fast)
        #拼接
        # cls_score = x_score_slow*status[0, 0] + x_score_fast*(1 - status[0, 0])
        if status[0, 0] > 0.5:
            cls_score = x_score_slow
        else:
            cls_score = x_score_fast
        # We do not predict bbox, so return None
        return cls_score, None
    @staticmethod
    ...
    def ava_pose_softmax_func(logits):
        # pose_logits = nn.Softmax(dim=1)(logits[:, :13])  #姿态行为  使用softmax，同一时刻同一主体不可能存在>=两种姿态
        # interact_logits = nn.Sigmoid()(logits[:, 13:])  #交互行为  使用sigmoid，同一时刻可能存在多种动作
        # logits = torch.cat([pose_logits, interact_logits], dim=1)
        # logits = torch.clamp(logits, min=0., max=1.)

        #0~15:  7-进食  13-饮水  14-抓食 是可能与其他动作组合的
        pose_logits = torch.cat([logits[:, :7], logits[:, 8:13], logits[:, 15:]], dim=1)  #[0 1 2 3 4 5 6    8 9 10 11 12     15 16]
...
if mmdet_imported:
    MMDET_HEADS.register_module()(monkey_BBoxHeadAVA_switch_group_linear)

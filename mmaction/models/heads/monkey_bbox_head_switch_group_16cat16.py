...
class monkey_BBoxHeadAVA_switch_group_16cat16(nn.Module):
    ...
    def __init__(
        ...
        super(monkey_BBoxHeadAVA_switch_group_16cat16, self).__init__()
        ...
        self.fc_cls1 = nn.Linear(in_channels, self.num_classes)
        self.fc_cls2 = nn.Linear(in_channels, self.num_classes)
    ...
    def forward(self, x, status):
        ...
        status = nn.Sigmoid()(status)
        cls_score = status[:,0:1] * cls_score_slow + status[:,1:2] * cls_score_fast
...
if mmdet_imported:
    MMDET_HEADS.register_module()(monkey_BBoxHeadAVA_switch_group_16cat16)

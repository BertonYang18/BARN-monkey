...
@PIPELINES.register_module()
class SampleMonkeyFrames(SampleFrames):

    def __init__(self, clip_len, frame_interval=2, test_mode=False):  #参数继承自 @PIPELINES.register_module()
    ...
    def __call__(self, results):  #compose.py中__call__(self, data): data= t(data),  t_1==SampleMonkeyFrames  这里的data==参数results
        fps = results['fps']
        timestamp = results['timestamp']
        timestamp_start = results['timestamp_start']
        shot_info = results['shot_info']

        center_index = int(fps * timestamp + 0.1) 
        # center_index = int(fps * (timestamp - timestamp_start)) 
        # center_index = int(fps * (timestamp - 900)) 
        skip_offsets = np.random.randint(
            -self.frame_interval // 2, (self.frame_interval + 1) // 2,
            size=self.clip_len)
        frame_inds = self._get_clips(center_index, skip_offsets, shot_info)
...
class RawFrameDecode:
    ...
    def __call__(self, results):
        ...
        for i, frame_idx in enumerate(results['frame_inds']):
            ...
            if frame_idx in cache:  #同一帧中第2+的bbox对应的相同帧，不再打开文件，而是复制第一帧的保存结果
                if modality == 'RGB':
                ...
                cache[frame_idx] = i  # frame_idx==1475 对应着 i==0

            frame_idx += offset
            if modality == 'RGB':
                filepath = osp.join(directory, filename_tmpl.format(frame_idx))
                # if filepath == '/data/ys/monkey/alldata/labels/all/b9_up_14_0940_0953/frames_000392.jpg':
                #     print('')
                try:
                    ...
                    print("results['shot_info']", results['shot_info'])
                    img_bytes = self.file_client.get(filepath)
                # Get frame with channel order RGB directly.
                cur_frame = mmcv.imfrombytes(img_bytes, channel_order='rgb')  # (1080,1920,3)
                imgs.append(cur_frame)
            elif modality == 'Flow':
        ...
        # imgs[i]==RGB_Data  和 results['frame_inds'][i] 对应
        results['imgs'] = imgs
        results['original_shape'] = imgs[0].shape[:2]
        results['img_shape'] = imgs[0].shape[:2]
        ...
        if 'gt_bboxes' in results:
            ...
            gt_bboxes = results['gt_bboxes']  #相对坐标
            gt_bboxes = (gt_bboxes * scale_factor).astype(np.float32)  #绝对坐标

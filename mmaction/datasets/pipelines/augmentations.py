...
class RandomCrop:
    ...
    def __call__(self, results):
        ...
        y_offset = 0
        x_offset = 0  # img_h和img_w的短边h/w为随机选的285 in （256，320）.长边w/h > h/w
        if img_h > self.size:
            y_offset = int(np.random.randint(0, img_h - self.size))    # randint(0，32) == 22
        if img_w > self.size:
            x_offset = int(np.random.randint(0, img_w - self.size))    # randint(0，251) == 4
        ...
        old_crop_quadruple = results['crop_quadruple']  # array([0,0,1,1]) --  (x_offset, y_offset, w, h): radio
...
class Resize:
    ...
    def __call__(self, results):
        ...
        if self.keep_ratio:
            new_w, new_h = mmcv.rescale_size((img_w, img_h), self.scale) #得到缩放因子，相乘得到****new_w, h****
        else:
            new_w, new_h = self.scale
        # 由mmcv库得到的new_w,h推算出 缩放因子
        self.scale_factor = np.array([new_w / img_w, new_h / img_h],
                                     dtype=np.float32)

        results['img_shape'] = (new_h, new_w)
        results['keep_ratio'] = self.keep_ratio  #保持横纵比
...
class RandomRescale:
    ...
    def __call__(self, results):
        ...
        short_edge = np.random.randint(self.scale_range[0],
                                       self.scale_range[1] + 1)  # 从(256,320+1)中选取一个整数，作为短边。 如285
...
class Flip:
    ...
    def __call__(self, results):
        ...
        if 'gt_bboxes' in results and flip:  # 翻转 gt_bboxes, proposals
...
class Normalize:
    ...
    def __call__(self, results):
        ...
        if modality == 'RGB':
            n = len(results['imgs'])  #clips预设的长度，如 11

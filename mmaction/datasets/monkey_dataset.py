...
from ..core.evaluation.ava_utils_monkey import ava_eval, read_labelmap, results2csv
...
class MonkeyDataset(BaseDataset):
    ...
    def __init__(self,
                 ...
                 timestamp_start='no work',
                 timestamp_end='no work',
                 fps=30):
        ...
        # 处理后的anno数据保存在self.video_infos中；  预测的bbox结果保存在self.proposals中
        # 构建/第一遍访问 dataset时是不涉及图片帧的构建的。
    ...
    def parse_img_record(self, img_records):
        ...
        bboxes = np.stack(bboxes)  # [array(4,), array(4,)] -> array(2,4)
    ...
    def load_annotations(self):
        ...
        with open(self.ann_file, 'r') as fin:
            for line in fin:
                ...
                # if '.' in line_split[1]:
                #     f_point, b_point = line_split[1].split('.')
                #     if len(b_point) > 1:  #过滤0.3333  0.6667
                #         continue
                # timestamp = int(float(line_split[1]))
                timestamp = float(line_split[1])
                img_key = f'{video_id},{timestamp:.6f}'
                # img_key = f'{video_id},{timestamp:06d}'

                num_frame_end= int(line_split[8])
                entity_box = np.array(list(map(float, line_split[2:6])))
                entity_id = int(line_split[7]) 
                # shot_info = (0, (self.timestamp_end - self.timestamp_start) * self._FPS)
                shot_info = (1, num_frame_end)
        ...
        for img_key in records_dict_by_img:
            ...
            video_info = dict(
                frame_dir=frame_dir,
                video_id=video_id,
                timestamp=float(timestamp),  #timestamp=int(timestamp)
                img_key=img_key,
                shot_info=shot_info,
                fps=self._FPS,
                ann=ann)

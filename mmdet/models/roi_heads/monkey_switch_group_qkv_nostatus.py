...
class monkey_switch_group_QKV_nostatus(nn.Module):
    def __init__(self, status_dim=2, input_dim=2304, hidden_dim=512, kernel_size=3, mlp_1x1=False):
        super(monkey_switch_group_QKV_nostatus, self).__init__()
        ...
        self.conv_q2 = nn.Conv2d(hidden_dim, hidden_dim, kernel_size, padding=padding, bias=False)
        #k,v
    ...
    def forward(self, status, rois, bbox_feat):
        ...
        query1 = self.conv_q1(bbox_feat).unsqueeze(1)  # ->(124,512,8,8) ->(124,1,512,8,8)
        query2 = self.conv_q2(bbox_feat).unsqueeze(1)  # ->(124,512,8,8) ->(124,1,512,8,8)
        #k1, v1   

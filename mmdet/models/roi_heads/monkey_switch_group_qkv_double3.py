...
class monkey_switch_group_QKV_double3(nn.Module):
    def __init__(self, status_dim=2, input_dim=2304, hidden_dim=512, kernel_size=3, mlp_1x1=False):
        super(monkey_switch_group_QKV_double3, self).__init__()
    ...
    def forward(self, status, rois, bbox_feat):
        ...
        #q1 q2
        #query = self.conv_q(status).unsqueeze(1)
        query21 = self.conv_q21(status).unsqueeze(1)  # ->(124,512,8,8) ->(124,1,512,8,8)
        query22 = self.conv_q22(status).unsqueeze(1)  # ->(124,512,8,8) ->(124,1,512,8,8)
        #k1, v1   

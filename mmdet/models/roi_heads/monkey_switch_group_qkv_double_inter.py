...
class monkey_switch_group_QKV_double_inter(nn.Module):
    def __init__(self, status_dim=2, input_dim=2304, hidden_dim=512, kernel_size=3, mlp_1x1=False):
        super(monkey_switch_group_QKV_double_inter, self).__init__()
        ...
        '''第二阶段'''
        #self.conv_reduce2 = nn.Conv2d(hidden_dim*2, hidden_dim, kernel_size, padding=padding, bias=False) #512*2 ->512
        #q
        self.conv_q21 = nn.Conv2d(status_dim, hidden_dim, kernel_size, padding=padding, bias=False)  #***hidden_dim
        self.conv_q22 = nn.Conv2d(status_dim, hidden_dim, kernel_size, padding=padding, bias=False)  #***hidden_dim
        #k,v
    ...
    def forward(self, status, rois, bbox_feat):
        ...
        status_slow = torch.cat((status[:, 0].reshape(B,1), rois[:, 1:]), dim=1)  #(124,1) + (124,4) -> (124, 5)
        status_fast = torch.cat((status[:, 1].reshape(B,1), rois[:, 1:]), dim=1)  #(124,1) + (124,4) -> (124, 5)
        #status = rois[:, 1:]
        status_slow = status_slow.unsqueeze(2).unsqueeze(2).expand(B,self.status_dim,H,W)  # ->(124,5,8,8)
        status_fast = status_fast.unsqueeze(2).unsqueeze(2).expand(B,self.status_dim,H,W)  # ->(124,5,8,8)

        '''第一阶段'''
        #q1 q2
        query1 = self.conv_q(bbox_feat).unsqueeze(1)
        ...
        # virt_feats2 = self.conv(virt_feats2)
        ...
        # group_bbox_feat_first = torch.cat([group_bbox_feat1, group_bbox_feat2], dim=1)  # (124,1024,8,8)
        group_bbox_feat_first = group_bbox_feat1  # (124,512,8,8)  可取消self.conv_reduce2

        '''第二阶段'''
        #status_slow, status_fast: (124,5,8,8)

        #group_bbox_feat_first = self.conv_reduce2(group_bbox_feat_first) 
        #q1 q2
        #query = self.conv_q(status).unsqueeze(1)
        query21 = self.conv_q21(status_slow).unsqueeze(1)  # (124,5,8,8)->(124,512,8,8) ->(124,1,512,8,8)
        query22 = self.conv_q22(status_fast).unsqueeze(1)  # (124,5,8,8)->(124,512,8,8) ->(124,1,512,8,8)
        #k1, v1   
        key21 = self.conv_k21(group_bbox_feat_first).unsqueeze(0)  # -> (1，124,512,8,8)
        att21 = (query21 * key21).sum(2) / (self.hidden_dim ** 0.5) # (124，124,8,8)
        att21 = nn.Softmax(dim=1)(att21)
        value21 = self.conv_v21(group_bbox_feat_first)  # (124,512,8,8)
        virt_feats21 = (att21.unsqueeze(2) * value21).sum(1)   # (124,512,8,8)
        #k2, v2
        key22 = self.conv_k22(group_bbox_feat_first).unsqueeze(0)  # -> (1，124,512,8,8)
        att22 = (query22 * key22).sum(2) / (self.hidden_dim ** 0.5) # (124，124,8,8)
        att22 = nn.Softmax(dim=1)(att22)
        value22 = self.conv_v22(group_bbox_feat_first)  # (124,512,8,8)
        ...
        #bbox_feat1  慢
        group_bbox_feat_second1 = group_bbox_feat_first + virt_feats21 # (124,512,8,8) +=  (124,512,8,8)
        
        #bbox_feat2  快
        group_bbox_feat_second2 = group_bbox_feat_first + virt_feats22 # (124,512,8,8) +=  (124,512,8,8)


        group_bbox_feat_second1 = group_bbox_feat_second1.unsqueeze(2)  # (124,512,1,8,8)
        group_bbox_feat_second2 = group_bbox_feat_second2.unsqueeze(2)  # (124,512,1,8,8)
        group_bbox_feats_second = [group_bbox_feat_second1, group_bbox_feat_second2]
        return group_bbox_feats_second

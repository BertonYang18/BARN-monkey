...
def _scale_size(size, scale):
    ...
    if isinstance(scale, (float, int)):
        scale = (scale, scale)
    w, h = size
    return int(w * float(scale[0]) + 0.5), int(h * float(scale[1]) + 0.5) #+0.5便于四舍五入
...
def rescale_size(old_size, scale, return_scale=False):
    ...
    if isinstance(scale, (float, int)):
        ...
        scale_factor = min(max_long_edge / max(h, w),
                           max_short_edge / min(h, w))  #因为max_long_edge==np.inf即无穷大，故而实现了短边对齐
    else:
        raise TypeError(
            f'Scale must be a number or tuple of int, but got {type(scale)}')

    new_size = _scale_size((w, h), scale_factor)  #w, h * scale_factor；四舍五入--int(float + 0.5)。因为 int()本身是直接去掉小数点后的数
